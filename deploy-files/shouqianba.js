...
export async function activateTerminal(deviceId, code) {
  ...
  // 响应数据在 biz_response 中
  const bizData = result.biz_response || result;

  if (result.result_code !== '200') {
    throw new Error(result.error_message || result.biz_response?.error_message || '激活失败');
  }

  return {
    terminalSn: bizData.terminal_sn,
    terminalKey: bizData.terminal_key,
    merchantSn: bizData.merchant_sn,
    storeSn: bizData.store_sn
  };
}
...
export async function checkinTerminal(terminalSn, terminalKey, deviceId) {
  ...
  return {
    terminalSn: result.biz_response?.terminal_sn || result.terminal_sn,
    terminalKey: result.biz_response?.terminal_key || result.terminal_key
  };
}
