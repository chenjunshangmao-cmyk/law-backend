...
async function test() {
  ...
  console.log('   token长度:', token.length, 'token:', token.substring(0, 40) + '...');
  ...
  if (pay.data?.success) {
    ...
    console.log('   会员ID应已自动分配');
  } else {
    console.log('\n❌ 失败:', pay.data?.error, '(code:', pay.data?.code, ')');
  }
}
